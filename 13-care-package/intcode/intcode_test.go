...
func TestExecuteProgram(t *testing.T) {
	...
	for _, c := range cases {
		...
		go ExecuteProgram(c.input, memoryChannel, inputQueue, outputQueue, 0)
...
func TestExecuteProgramInput(t *testing.T) {
	...
	for _, c := range cases {
		...
		go ExecuteProgram(c.inputProgram, memoryChannel, inputQueue, outputQueue, 0)
...
func TestExecuteProgramOutput(t *testing.T) {
	...
	go ExecuteProgram(input, memoryChannel, inputQueue, outputQueue, 0)
...
func TestCompleteInputOutputPrograms(t *testing.T) {
	...
	for _, c := range cases {
		...
		go ExecuteProgram(c.program, memoryChannel, inputQueue, outputQueue, 0)
...
func TestExecuteProgramRelativeParameterMode(t *testing.T) {
	...
	for _, c := range cases {
		...
		go ExecuteProgram(c.input, memoryChannel, inputQueue, outputQueue, 128)
...
func fetchAllElementsFromQueue(queue queue.Queue) []int64 {
	values := []int64{}
	for queue.GetLen() > 0 {
		element, _ := queue.Dequeue()
		value, _ := element.(int64)
