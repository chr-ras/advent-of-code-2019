...
func TestFindClosestCrossingDistance(t *testing.T) {
	cases := []struct {
		...
		expectedClosestDistance int
		expectedOptimalDistance int
	}{
		{
			"R8,U5,L5,D3",
			"U7,R6,D4,L4",
	...
	for _, c := range cases {
		actualClosestDistance, actualOptimalDistance := FindClosestCrossingDistance(c.firstPath, c.secondPath)

		if actualClosestDistance != c.expectedClosestDistance || actualOptimalDistance != c.expectedOptimalDistance {
			t.Errorf("FindClosestCrossingDistance({%v}, {%v}) == (%v, %v), expected (%v, %v)", c.firstPath, c.secondPath, actualClosestDistance, actualOptimalDistance, c.expectedClosestDistance, c.expectedOptimalDistance)
		}
	}
}
