...
func TestCalcHighestSignal(t *testing.T) {
	cases := []struct {
		input    []int
		expected int
	}{
	...
	for _, c := range cases {
		actual := CalcHighestSignal(c.input)

		if actual != c.expected {
			t.Errorf("CalcHighestSignal(%v) == %v, expected %v", c.input, actual, c.expected)
		}
	}
}

func TestCalcHighestSignalInFeedbackLoopMode(t *testing.T) {
	cases := []struct {
		input    []int
		expected int
	}{
	...
	for _, c := range cases {
		actual := CalcHighestSignalInFeedbackLoopMode(c.input)
